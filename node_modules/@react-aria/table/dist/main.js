...
function $16d645f9e2153641$export$52994e973806c219(props, state, ref) {
    let { column: item , showResizer: showResizer  } = props;
    ...
    const { moveProps: moveProps  } = $kJQf8$reactariainteractions.useMove({
        ...
        onMove ({ deltaX: deltaX , pointerType: pointerType  }) {
            ...
            if (deltaX !== 0) {
                ...
                if (stateRef.current.getColumnMinWidth(item.key) >= stateRef.current.getColumnWidth(item.key)) document.body.style.setProperty('cursor', direction === 'rtl' ? 'w-resize' : 'e-resize');
                else if (stateRef.current.getColumnMaxWidth(item.key) <= stateRef.current.getColumnWidth(item.key)) document.body.style.setProperty('cursor', direction === 'rtl' ? 'e-resize' : 'w-resize');
                else document.body.style.setProperty('cursor', 'col-resize');
            }
        },
        onMoveEnd ({ pointerType: pointerType  }) {
    ...
    return {
        resizerProps: {
            ...$kJQf8$reactariautils.mergeProps(moveProps, {
                onFocus: ()=>{
